...
class OTSim(AppBase):
                    ...
                    if helics_app:
                        ...
                        if root_broker:
                            if "|" in root_broker:
                                ...
                                root_broker = self.extract_node_interface_ip(
                                    hostname, iface
                                )
                            elif ":" in root_broker:
    ...
    def __init_defaults(self):
        self.default_infrastructure: str = self.metadata.get(
        ...
        if "helics" in self.metadata:
            ...
            self.default_endpoint: str = self.metadata["helics"].get(
                "endpoint", f"{self.default_fed}/updates"
            )
    ...
    def pre_start(self):
        logger.info(f"Starting ASHWINI1 user application: {self.name}")
            ...
            if len(devices) > 0:
                ...
                if "helics" in md:
                    ...
                    if "federate" in md["helics"]:
                        if isinstance(md["helics"]["federate"], str):
                            ...
                            federate.text = md["helics"]["federate"].get(
                                "name", server.hostname
                            )
                            log_level.text = md["helics"]["federate"].get(
                                "log-level", "SUMMARY"
                            )
                    else:
                ...
                if not self.extract_app("helics") and ":24000" not in addr:
                    ...
                    self.add_inject(
                        ...
                        inject={"src": broker_addr_wait[addr], "dst": dst},
                    )
